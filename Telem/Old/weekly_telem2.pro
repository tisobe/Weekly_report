PRO WEEKLY_TELEM2, tstart,tstop
  ...
  if (n_elements(files) gt 1) then begin
    for ifiles=0, n_elements(files)-1 do begin
      ...
      for itelem=0,n_elements(telem)-1 do begin
        if (telem(itelem).yellow gt 0.1 or telem(itelem).red gt 0.1) then begin
          ...
          if (bnum eq 1) then begin
            ...
            lim = limits(name,describe=des,unit=units)
            lim_db=lim(n_elements(lim)-1)
            if (viol gt lim_db.yel_hi) then begin
...
if (name eq 'AWD3TQI') then print,files(ifiles),name,viol,lim_db.yel_lo
  ...
  day=day+1
print,date," ***********************************"
  if (date eq 20121126) then date=20121127
...
for iout=0,n_elements(maxarr(0,*))-1 do begin
  if (maxarr(8,iout) eq 1 or minarr(8,iout) eq 1) then begin
    ...
    b=where(minarr(1:7,iout) ne "x",bnum)
    if (bnum gt 0) then printf, ounit, minarr(0,iout), " | ",viols
    viols=strjoin(maxarr(1:7,iout)," | ")
    b=where(maxarr(1:7,iout) ne "x",bnum)
    if (bnum gt 0) then printf, ounit, maxarr(0,iout), " | ",viols
  endif
